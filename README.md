...
| **安装包（安装后使用）** | 运行下载的安装包，完成安装后打开 PhotoMap | 希望像普通 Windows 软件一样安装使用 |

前往 **[Releases 下载页面](https://github.com/bytwelve/photoMap/releases/latest)** 获取 Windows x64 安装包或便携版 ZIP，按上表使用，无需安装 Node.js 或运行 npm 命令。下载页面同时提供文件校验值和版本说明；也可以选择[从源码构建](#从源码构建)。
